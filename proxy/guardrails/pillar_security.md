...
<Tip title="**Enable `plr_mask: true`** to automatically sanitize sensitive data (PII, secrets, payment card info) before it reaches the LLM. Masked content is replaced with placeholders while original data is preserved in Pillar's audit logs.">
...
<Tabs>
<Tab title="Recommended (Dual Mode)">
...
<Tab title="Input-Only Protection">
...
<Tab title="Low Latency Parallel">
...
<Tip title="**Always set `plr_scanners: true` and `plr_evidence: true`** to see what Pillar detected. This is essential for troubleshooting and understanding security threats.">
