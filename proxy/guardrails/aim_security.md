...
<Note title="In case you want to host your guard on-premise, you can enable this option">
...
<Info title="When using LiteLLM with virtual keys, key-specific policies can be set directly in Aim's guards page by specifying the virtual key alias when creating the guard.">
...
<Note title="The following example depends on enabling *PII* detection in your guard.">
You can adjust the request content to match different guard's policies.
</Note>

<Tabs>
<Tab title="Successfully blocked request">

<Note title="When using LiteLLM with virtual keys, an `Authorization` header with the virtual key is required.">
...
<Tab title="Successfully permitted request">
