...
<Note title="For OpenAI/Anthropic Prompt Caching, go [here](../completion/prompt_caching.md)">
...
<Note title="Example python script for setting redis cluster nodes in .env:">
...
<Info title="IAM authentication for redis is only supported via GCP and only on Redis Clusters for now.">
...
**Additional kwargs**  
<Info title="Use `REDIS_*` environment variables to configure all Redis client library parameters. This is the suggested mechanism for toggling Redis settings as it automatically maps environment variables to Redis client kwargs.">
...
<Warning title="**Note**: For non-string Redis parameters (like integers, booleans, or complex objects), avoid using `REDIS_*` environment variables as they may fail during Redis client initialization. Instead, use `cache_kwargs` in your router configuration for such parameters.">
...
<Tab title="Qdrant Semantic cache">
...
<Tab title="redis semantic cache">
