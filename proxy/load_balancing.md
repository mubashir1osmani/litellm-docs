...
<Info title="For more details on routing strategies / params, see [Routing](../routing.md)">
...
<Info title="By default, `rpm` and `tpm` values are only used for **routing decisions** (picking deployments with capacity). With `enforce_model_rate_limits`, they become **hard limits**.">
...
<Info title="Detailed information about [routing strategies can be found here](../routing)">
...
<Note title="The `order` parameter requires `enable_pre_call_checks: true` in `router_settings`.">
