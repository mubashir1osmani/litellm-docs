...
<Tip title="**We support ALL GigaChat models, just set `model=gigachat/<any-model-on-gigachat>` as a prefix when sending litellm requests**">

</Tip>

<Warning title="GigaChat API uses self-signed SSL certificates. You must pass `ssl_verify=False` in your requests.">
...
<Note title="Available models may vary depending on your API access level (personal or business).">
