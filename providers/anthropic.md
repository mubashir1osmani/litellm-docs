...
<Info title="When using structured outputs with supported models, LiteLLM automatically:">
...
<Tip title="Azure Foundry Support">
...
<Info title="**Finding your Azure endpoint:** Go to Azure AI Foundry → Your deployment → Overview. Your base URL will be `https://<resource-name>.services.ai.azure.com/anthropic`">
...
<Tab title="config - default all Anthropic Model">
...
<Note title="Here's what a sample Raw Request from LiteLLM for Anthropic Context Caching looks like:">
...
<Info title="LiteLLM Proxy is OpenAI compatible">
...
<Info title="The Anthropic Memory tool is currently in beta.">
...
<Note title="For Claude Opus 4.6, all `reasoning_effort` values (`low`, `medium`, `high`) are mapped to `thinking: {type: &quot;adaptive&quot;}`. To use explicit thinking budgets, pass the native `thinking` parameter directly:">
