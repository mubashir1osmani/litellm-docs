...
<Tip title="Gemini API vs Vertex AI">
...
<Info title="Note: Reasoning cannot be turned off on Gemini 2.5 Pro models.">

</Info>

<Tip title="Gemini 3 Models">
...
<Warning title="**Temperature Recommendation for Gemini 3 Models**">
...
<Info title="LiteLLM supports Gemini TTS models that can generate audio responses using the OpenAI-compatible `audio` parameter format.">
...
<Tab title="LiteLLM Proxy Server">
...
<Info title="**Per-Part Resolution:** Each image or video in your request can have its own `detail` setting, allowing mixed-resolution requests (e.g., a high-res chart alongside a low-res icon). This feature works with both `image_url` and `file` content types, and is only available for Gemini 3+ models.">
...
<Note title="**Field Name Conversion:** LiteLLM automatically converts snake_case field names to camelCase for the Gemini API:">
- `start_offset` → `startOffset`
- `end_offset` → `endOffset`
- `fps` remains unchanged
</Note>

<Warning title="- **Gemini 3+ Only:** This feature is only available for Gemini 3.0 and newer models">
...
<Tabs>
<Tab title="Basic Video Metadata">
...
<Tab title="Combined with Detail">
...
## Chat Models
<Tip title="**We support ALL Gemini models, just set `model=gemini/<any-model-on-gemini>` as a prefix when sending litellm requests**">
...
![](/images/gemini_context_caching.png)
...
<Tab title="Curl with Custom TTL">
...
<Tab title="OpenAI Python SDK with TTL">
