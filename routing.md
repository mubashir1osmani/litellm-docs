...
<Info title="If you want a server to load balance across different LLM APIs, use our [LiteLLM Proxy Server](./proxy/load_balancing.md)">
...
<Info title="See detailed proxy loadbalancing/fallback docs [here](./proxy/reliability.md)">
...
<Tabs>
<Tab title="(Default) Weighted Pick - RECOMMENDED">
...
<Tab title="Weight-based shuffling">
...
<Tab title="Rate-Limit Aware v2 (ASYNC)">
...
<Tab title="Custom Routing Strategy">
...
<Tab title="Lowest Cost Routing (Async)">
...
<Note title="The `order` parameter requires `enable_pre_call_checks=True` to be set on the Router.">
...
<Info title="When configuring Redis caching in router settings, use `cache_kwargs` to pass additional Redis parameters, especially for non-string values that may fail when set via `REDIS_*` environment variables.">
...
<Tabs>
<Tab title="Context Window Check">
...
<Info title="Go [here](./proxy/reliability.md#advanced---context-window-fallbacks) for how to do this on the proxy">
